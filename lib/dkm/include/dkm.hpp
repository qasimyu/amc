...
std::vector<std::vector<T>> calculate_means(const std::vector<std::vector<T>>& data,
	...
	for (size_t i = 0; i < k; i++) {
		std::vector<T> tmp;
		for (size_t j = 0; j < old_means[0].size(); j++) {
			tmp.push_back(0);
		}
		means.push_back(tmp);
	}
...
template <typename T>
T silhouette_score(const std::vector<std::vector<T>>& data,
	const std::vector<uint32_t>& clusters,
	uint32_t k) {
		
	uint32_t i, j, N = data.size();
	...
	T silhouette_avg = 0;
	for(i = 0; i < N; i++) {
		std::vector<T> sum_dists(k, 0);
		std::vector<uint32_t> counts(k, 0);
		for(j = 0; j < N; j++) {
			if(i == j)  continue;
			sum_dists[clusters[j]] += distances[i][j];
			counts[clusters[j]]++;
		}
		
		T intra_dist = (counts[clusters[i]] > 0)? sum_dists[clusters[i]]/counts[clusters[i]]:0;
		T min_inter_dist = numeric_limits<uint32_t>::max();
		for(j = 0; j < k; j++) {
			if(clusters[i] == j) continue;
			T inter_dist = sum_dists[j]/counts[j];
		...
		if(k == 1) {
			...
			silhouette_avg += (min_inter_dist-intra_dist)/max(intra_dist, min_inter_dist);
		}
	}
	/*
	if(k == 1) {
		return 1.0/silhouette_avg;
	}
	*/
	return silhouette_avg/data.size();
}
...
std::tuple<std::vector<std::vector<T>>, std::vector<uint32_t>> kmeans_lloyd(
	const std::vector<std::vector<T>>& data, const clustering_parameters<T>& parameters, T& total_dist, T& score) {
	...
	std::vector<T> distances = details::closest_distance(means, data);
	total_dist = 0;
	for(size_t i = 0; i < distances.size(); i++) {
		total_dist += distances[i];
	}
	score = details::silhouette_score(data, clusters, means.size());
